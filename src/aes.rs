...
/*fn genkey(k: u8) {
    const K: usize = k;
    let mut last_rand_char: [u8; K] = [];
    let pc = "-0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ_abcdefghijklmnopqrstuvwxyz";
    let mut lpt: u32 = 0;

    let gu = |mut x| -> String{
        let dup: bool = (x == lpt);
        lpt = x;
        let mut time_stamp_chars:&[u8] = &[];
        let mut result = b"".to_vec();
        for i in 7..0 {
            time_stamp_chars.get(i).unwrap() = pc.get(x%64).unwrap();
            x = x/64;
        }
        result.append(&mut time_stamp_chars.to_vec());

        if !dup {
            for i in 0..k {
                last_rand_char.get(i).unwrap() = rand::thread_rng().gen_range(0..64);
            }
        }else {
            || {
                for i in k-1..0 {
                    if last_rand_char.get(i).unwrap() != 63{
                        last_rand_char.get(i).unwrap() = last_rand_char.get(i).unwrap() +1;
                        return;
                    }
                    last_rand_char.get(i).unwrap() = &0;
                }
            }
        }

        for i in 0..k {
            result.push(*pc.get(last_rand_char.get(i).unwrap()).unwrap());
        }
